...
export default function AddProductModal({ isOpen, onClose, onProductCreated }: AddProductModalProps) {
  ...
  const handleAddProduct = async (e: FormEvent) => {
    ...
    try {
      ...
      console.log(`Preparing to upload file: ${modelFile.name} (${(modelFile.size / (1024 * 1024)).toFixed(2)} MB)`);
      ...
      if (!createProductResponse.ok) {
        throw new Error(`Failed to create product: ${createProductResponse.statusText}`);
      }
      ...
      <div className="bg-white p-6 rounded-sm border border-black shadow-lg w-full max-w-2xl">
        ...
        {uploadSuccess ? (
          <div className="p-4 mb-4 bg-green-50 border border-green-200 text-green-700 rounded-sm">
            <p className="font-montreal text-sm">Product created successfully!</p>
          </div>
        ) : (
          <form onSubmit={handleAddProduct} className="space-y-4">
            ...
            <div>
              <label htmlFor="category" className="block mb-1 font-montreal text-sm text-black">
                Category / Pose Type
              </label>
              <select
                id="category"
            ...
            <div>
              <label htmlFor="model_file" className="block mb-1 font-montreal text-sm text-black">
              ...
              <div className="border border-black p-4 rounded-sm">
                {modelFile ? (
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-montreal text-sm text-black">{modelFile.name}</span>
                      <span className="ml-2 text-xs text-gray-500">({(modelFile.size / (1024 * 1024)).toFixed(2)} MB)</span>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="flex flex-col items-center justify-center py-4">
                    ...
                    <label htmlFor="file-upload" className="cursor-pointer px-4 py-2 bg-black text-white font-montreal hover:bg-white hover:text-black border border-black transition-colors rounded-sm">
                      Choose File
                      <input
                        id="file-upload"
                        type="file"
